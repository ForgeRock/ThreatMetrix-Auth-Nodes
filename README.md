...
Due to changes in the APIs, the LexisNexis ThreatMetrix Nodes v1.2.1 are not compatible with versions 
...
* **Page ID** - (Optional) The Page ID is an identifier to be used if you place the ThreatMetrix tag on multiple pages.
...
The ThreatMetrix Update Node provides retrospective truth data to ThreatMetrix for an event. The typical ForgeRock Authentication Tree will perform a ThreatMetrix Query and if step-up authentication is involved, the ThreatMetrix Update Node is integrated to provide additional details on the event. Truth data is incredibly beneficial for tuning of the policy and overall fraud detection.

The ThreatMetrix Update Review Node has the following configuration parameters:
* **Org ID** - Org ID is the unique id associated with ThreatMetrix generated for your organization.
* **API Key** - This is the unique API key generated by ThreatMetrix associated to the Org ID.
* **Update URI** - ThreatMetrix Update URI.
* **Event Tag** - This represents the event disposition and outcome of a ThreatMetrix Risk Assessment. The following options are available
    - Step-Up Initialize - This sends <code>challenge_init</code> event tag associated to the ThreatMetrix Risk Assessment. The proper placement in an authentication tree is prior to sending a Step-Up authentication request in the event the transaction is abandoned.
    - Step-Up Pass - This sends <code>challenge_pass</code> event tag associated to the ThreatMetrix Risk Assessment. The proper placement in an authentication tree is following a Step-Up / MFA successful operation. Thus, a sequence of init and pass are seen as truth data for the ThreatMetrix Risk Assessment.  Additionally, this value will set final review status to pass.
    - Step-Up Fail - This sends <code>challenge_fail</code> event tag associated to the ThreatMetrix Risk Assessment. The proper placement in an authentication tree is following a Step-Up / MFA failure. Thus, a sequence of init and fail are seen as truth data for the ThreatMetrix Risk Assessment.  Additionally, this value will set final review status to fail.
    - Final Review Status Pass - This sends <code>final_review_status=pass</code> associated to the ThreatMetrix Risk Assessment. The proper placement for this event type within a authentication tree is at the point full success of the transaction has been determined.
    - Final Review Status Reject - This sends <code>final_review_status=reject</code> associated to the ThreatMetrix Risk Assessment. The proper placement for this event type within a authentication tree is at the point failure of the transaction has been determined.
